...
/**
 * represents the human player model
 */
public class HumanPlayer implements Player {
  ...
  private AiPlayerDependency aiDependencyModel = new AiPlayerDependency();
  ...
  public List<Coord> takeShots() {
    for (Coord c : inputs) {
      ...
      } else if (getCoord(c.getCoordX(), c.getCoordY(), opponentBoard).getStatus()
          == Status.EMPTY) {
        Coord b = getCoord(c.getCoordX(), c.getCoordY(), opponentBoardData);
        b.setStatus(Status.MISSED);
      }
    }
    aiDependencyModel.getAiPlayer().reportDamage(inputs);
    return inputs;
  }
   ...
   * @return the coordinate at the specified x and y value from the given board
   */
   ...
   *
   * @param aiDependencyModel The AI player.
   */
  public void setAiDependencyModel(AiPlayerDependency aiDependencyModel) {
    this.aiDependencyModel = aiDependencyModel;
  }
